...
def configure_in_chunks(name, total_size, id_offset=0):
    ...
    while i <= total_size:
        ...
        item_config = input("Dictonary for {} {}: ".format(chunk_size, name))
...
def main():
    ...
    num_edges = min(int(raw_input("Number of edges (max %d): " % CANARY_ID_OFFSET)),
                    CANARY_ID_OFFSET)

    max_canaries = 254 - CANARY_ID_OFFSET
    num_canaries = min(int(raw_input("Number of canaries (max: %d): " % max_canaries)),
                       max_canaries)
